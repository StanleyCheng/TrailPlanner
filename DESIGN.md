...
Circular ~120px magnifier floating just NW (above-left) of the pointer — clamped to the viewport, never under the finger/cursor (flips SE near the top-left corner). **Canvas-drawn 1.2× magnification** — tiles are redrawn onto a canvas via getBoundingClientRect geometry (no cloned panes, no CSS-compositing dependence), so the disc paints deterministically on every browser; no second map instance, no extra tile requests. Crosshair marks the pin tip's ground point (the coordinate is the teardrop tip, not the icon center); the disc is border-free (box-shadow ring) so the crosshair centre displays exactly the ground point under the pin's tip — what you see under the crosshair is where the pin lands. No pin graphic inside the loupe. Entries: press-and-hold ~200ms to place (touch, mouse, and pen) and dragging an existing pin (any pointer type; loupe shows for the drag duration only). iOS: suppress callout/selection on the map (`-webkit-touch-callout:none; user-select:none`) and `preventDefault` contextmenu during the gesture.
