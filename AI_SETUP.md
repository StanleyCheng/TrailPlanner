...
1. Sign in at [Vercel](https://vercel.com/) with the GitHub account that can access `StanleyCheng/TrailPlanner`.
2. Choose **Add New → Project**, import `StanleyCheng/TrailPlanner`, keep the repository root as the Root Directory and choose the Hobby plan only for personal, non-commercial use. `vercel.json` selects the Other framework preset, build command and `dist` output.
...
To keep using GitHub Pages, enter `https://trailplanner.vercel.app/api/recognize-map` and the owner code in its private connection fields. Both stay only in browser memory for the current visit. The Vercel function accepts the Vercel page on the same origin and only the extra origins listed in `ALLOWED_ORIGIN`.
