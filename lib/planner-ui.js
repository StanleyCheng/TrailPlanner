    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      try {
        ...
        routing.busy = false; $('routing-status').textContent = `Found ${routing.result.routes.length} option(s) through every waypoint. Select one below to review before export.`; showRoutes(); render(); setDockExpanded(false); toast((routing.result.notices || [])[0] || `Found ${routing.result.routes.length} route option${routing.result.routes.length === 1 ? '' : 's'} through every waypoint.`, false, routing.result.notices?.length ? 9000 : 7000);
