    ...
    const DIFFICULTY_STYLES = [
      { color: '#13834b', label: 'Normal hiking', rank: 0 },
      { color: '#e34e9b', label: 'More demanding hiking', rank: 1 },
      { color: '#d00000', label: 'Difficult / alpine hiking', rank: 2 }
    ];
    ...
    function showRoutes() {
      ...
      routing.result.routes.forEach((route, i) => {
        ...
        if (route.harderTerrainMetres > 0) facts.append(element('li', harderTerrainWarning(route), 'terrain-warning'));
        if (route.roughSurfaceMetres > 0) facts.append(element('li', roughSurfaceWarning(route)));
        for (const text of [`All ${state.points.length} places · order ${route.order.map(i => i + 1).join(' → ')}`, route.loop ? `${km(route.end.approach)} final mapped leg back to waypoint 1` : `${km(route.start.approach)} approach / ${km(route.end.approach)} exit walk`, `${km(route.roadMetres)} mapped roads · ${km(route.repeatedMetres)} retraced`]) facts.append(element('li', text));
        const worstSnap = route.snaps.reduce((worst, snap) => snap.metres > worst.metres ? snap : worst);
        facts.append(worstSnap.metres > 15
          ? element('li', `Largest waypoint offset: pin ${worstSnap.index + 1} “${worstSnap.original.name}” is ${Math.ceil(worstSnap.metres)} m from the routed path. The route follows the nearest eligible mapped way; raise the waypoint tolerance or move that pin onto a mapped path to pass closer.`, 'terrain-warning')
          : element('li', `Largest waypoint offset: ${Math.ceil(worstSnap.metres)} m`));
    ...
    function routeDetails() {
      ...
      if (r.harderTerrainMetres > 0) trails.append(element('p', harderTerrainWarning(r), 'terrain-warning'));
      if (r.roughSurfaceMetres > 0) trails.append(element('p', roughSurfaceWarning(r)), externalLink('About OSM surface smoothness', 'https://wiki.openstreetmap.org/wiki/Key:smoothness'));
    ...
    function paintPlannedRoute() {
      ...
      if (map) {
        routes.forEach((route, i) => {
          ...
          if (!byDifficulty) L.polyline(route.coords.map(p => [p.lat, p.lon]), { color, weight: selected ? 7 : 5, opacity: selected ? 1 : .78 }).bindPopup(`Route ${i + 1} · ${route.title} · ${km(route.metres)}`).addTo(tracks);
        ...
        if (byDifficulty) for (const section of visibleDifficultySections(routes)) {
          L.polyline(section.coords.map(p => [p.lat, p.lon]), { color: section.color, weight: routing.selected?.id === section.routeId ? 7 : 5, opacity: 1 })
            .bindPopup(`Route ${section.routeNumber} · ${section.label} · ${sacScaleLabel(section.sacScale)} · ${Math.ceil(section.metres)} m`).addTo(tracks);
        }
        startRouteRun(routes);
      }
      const r = routing.selected; if (!r) { statusSetRoute(`${routes.length} proposed route${routes.length === 1 ? '' : 's'} · choose any combination below`); return; }
      const routeNumber = routes.findIndex(route => route.id === r.id) + 1;
      statusSetRoute(`Route ${routeNumber} · ${km(r.metres)} · ${r.title} · provisional · all ${state.points.length} mandatory places`);
    ...
    async function getBackendPlan(points, settings, provider, region, signal) {
      ...
      if (response.status === 422 && response.headers.get('X-TrailPlanner-Route-Policy') !== String(TrailRouter.ROUTING_POLICY_VERSION)) {
        const error = new Error('The route service rejected this plan under a different path policy. Retrying with the local engine.');
      ...
      if (payload.result.policyVersion !== TrailRouter.ROUTING_POLICY_VERSION) {
        const error = new Error('The route service uses a different path policy. Using the local engine for consistent access and terrain rules.');
        error.directFallback = true; throw error;
      }
      if (settings.allowHarderHiking && (payload.result.settings?.allowHarderHiking !== true || payload.result.routes.some(route => !Array.isArray(route.edges) || route.edges.length !== route.coords?.length - 1 || route.edges.some(edge => !Object.hasOwn(edge, 'sacScale'))))) {
        const error = new Error('The route service does not support harder hiking paths yet. Using the local engine for this setting.');
        error.directFallback = true; throw error;
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
          ...
          worker.onmessage = e => {
            ...
            if (e.data.progress) { $('routing-status').textContent = e.data.progress; statusFindDetail(e.data.progress, serial); }
            else {
              cleanup();
              e.data.error ? reject(Object.assign(new Error(e.data.error), { code: e.data.code, endpointIndices: e.data.endpointIndices })) : resolve(e.data.result);
            }
          };
          worker.onerror = () => { cleanup(); reject(new Error('The route worker could not run. Try a current browser or a smaller search.')); };
    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      $('map-route-toolbar').hidden = true; $('map-route-dots').replaceChildren(); $('map-export-action').disabled = true;
      toast(loopEnabled() ? 'Finding a mapped loop in pin order first. Transport is not required.' : 'Finding routes in pin order first. Transport: 1 km first, extending up to 20 km if needed…', false, 9000);
      ...
      try {
        ...
        try {
          $('routing-status').textContent = settings.loop ? 'Checking mapped paths through all pins and back to waypoint 1; transport is ignored for this loop…' : 'Checking mapped paths and transport: within 1 km first, then up to 20 km if needed…'; statusFindMilestone($('routing-status').textContent, serial);
          ...
          $('routing-status').textContent = 'Using the local route engine. Downloading paths from map providers…'; statusFindMilestone($('routing-status').textContent, serial);
          ...
          for (const radius of TrailRouter.TRANSPORT_EXPANSION_STEPS) {
            ...
            const expansionText = `${settings.loop ? 'No shared loop stop yet' : 'No transport at an end yet'}. Checking connected mapped approaches within ${radius / 1000} km…`;
        ...
        routing.busy = false; $('routing-status').textContent = `Found ${routing.result.routes.length} option(s) through every waypoint. Select one below to review before export.`; showRoutes(); render(); setDockExpanded(false); statusCairnCelebrate(); toast((routing.result.notices || [])[0] || `Found ${routing.result.routes.length} route option${routing.result.routes.length === 1 ? '' : 's'} through every waypoint — happy exploring! 🎉`, false, routing.result.notices?.length ? 9000 : 7000);
        ...
        $('routing-status').textContent = text; $('route-options').replaceChildren(element('div', text, 'routes-empty')); $('route-result-count').textContent = 'No qualifying routes'; statusBannerShow(text, { stage: 'routes' });
    ...
    $('cancel-routing').addEventListener('click', () => { stopRouting(); statusBannerClear(); $('routing-status').textContent = 'Search cancelled. Your waypoints are unchanged.'; $('route-options').replaceChildren(element('div', 'Search cancelled. Review your places and search again when ready.', 'routes-empty')); toast('Search cancelled. Your waypoints are unchanged.'); updateGuidance(); updateDockState(); });
    ...
    $('map-export-action').addEventListener('click', () => {
      ...
      toast(`${routes.length} shown route${routes.length === 1 ? '' : 's'} exported as GPX — happy trails! 🥾`); track('gpx_exported');
    });
    // END PLANNER UI
