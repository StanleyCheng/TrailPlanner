    ...
    function applySnap(animate = true) {
      ...
      document.documentElement.style.setProperty('--dock-visible-height', `${Math.ceil(sheet.heights.full - sheet.y)}px`);
    }
    ...
    function updateDockState() {
      ...
      setBarStatus(`Step ${currentIndex + 1} of 5 · ${currentStage === 'input' ? inputLabel : stageNames[currentStage]}`);
    ...
    function activateStage(stage, { focus = false } = {}) {
      ...
      if (reason) { setBarStatus(`${stageNames[stage]} is locked — ${reason}`); return; }
