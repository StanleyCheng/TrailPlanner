// BEGIN LOUPE UI
    ...
    // The loupe renders a pin replica whose tip sits exactly on the crosshair centre — keep its number in sync.
    function loupeSyncPinLabel() {
      const label = loupeEl?.querySelector('.loupe-pin span'); if (!label) return;
      label.textContent = loupeDraggedMarker?.getElement()?.querySelector('span')?.textContent || (loupeDraggedIndex >= 0 ? String(loupeDraggedIndex + 1) : '');
    }
    function loupeShow(latlng) { if (!latlng || !loupeEl) return; loupeSyncPinLabel(); loupeEl.hidden = false; loupeRefreshContent(); loupePlace(latlng); }
    ...
    function initLoupe(mapInstance) {
      ...
      loupeEl.querySelector('.loupe-pin path').setAttribute('d', WAYPOINT_PIN_PATH);
