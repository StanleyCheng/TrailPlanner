// BEGIN LOUPE UI
    ...
    //   x, y, pointerId, pointerType, adding, onMarker }
    ...
    function loupeGestureReduce(gesture, event) {
      ...
      if (event.type === 'down') {
        ...
        if (event.pointerType !== 'touch' || !event.adding || event.onMarker) return stay();
        return { gesture: { phase: 'pending', pointerId: event.pointerId, startX: event.x, startY: event.y }, effects: [] };
      }
      if (event.type === 'dragstart') {
        if (gesture.phase !== 'idle' || event.pointerType !== 'touch') return stay();
    ...
    // END LOUPE STATE
    const LOUPE_RADIUS = 60, LOUPE_SCALE = 2, LOUPE_LIFT = 100;
    ...
    let loupeDraggedMarker = null, loupeDraggedIndex = -1, loupeDownLatLng = null, loupeLastPointerType = 'mouse';
    ...
    function loupePlace(latlng) {
      ...
      let top = cy - LOUPE_LIFT - size;
      if (top < 8) top = cy + LOUPE_LIFT; // not enough room above the finger: flip below
      top = Math.min(top, window.innerHeight - size - 8);
      loupeEl.style.left = Math.round(Math.min(Math.max(cx - LOUPE_RADIUS, 8), window.innerWidth - size - 8)) + 'px';
    ...
    function loupeApplyEffects(effects, domEvent) {
      for (const effect of effects) {
        if (effect === 'place-pin') {
          if (!loupeDownLatLng) continue;
          addMapPinAt(loupeDownLatLng);
    ...
    function initLoupe(mapInstance) {
      ...
      container.addEventListener('pointerdown', event => {
        ...
        loupeApplyEvent({ type: 'down', x: event.clientX, y: event.clientY, pointerId: event.pointerId, pointerType: event.pointerType, adding: !!state.adding, onMarker: !!event.target.closest('.leaflet-marker-icon') }, event);
      ...
      for (const type of ['pointerup', 'pointercancel']) container.addEventListener(type, event => {
        if (loupeGesture.phase === 'idle' || loupeGesture.phase === 'pin-drag') return;
        loupeApplyEvent({ type: type === 'pointerup' ? 'up' : 'cancel', pointerId: event.pointerId }, event);
      });
